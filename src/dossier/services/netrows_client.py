"""Cliente HTTP mínimo para la API REST de Netrows (backend)."""
...
class NetrowsClient:
    def __init__(
        ...
        if not key:
            raise ValueError(
                "Falta NETROWS_API_KEY en el entorno (.env). "
                "Configura la clave de Netrows para búsquedas de personas."
            )
        self._api_key = key
